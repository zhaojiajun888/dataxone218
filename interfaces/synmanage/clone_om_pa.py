...
class Clone_om(Publicmethods):
    """
        oracle-mysql，一对一，不开启分离
        在savelink5_om_pa,停止运行后进行克隆,删除
        实现:克隆,删除克隆数据
    """
    def __init__(self):
        super(Clone_om, self).__init__()
    ...
    def clonePreCheck_om(self):
        ...
        try:
            ...
            if self.name1 == rename1 and self.name2 == rename2:
                loggers.info("Clone_om_pa.clonePreCheck_om执行成功".center(60, "~"))
            else:
                loggers.info("Clone_om_pa.clonePreCheck_om执行失败".center(60, "!"))
        except Exception as e:
            loggers.info("Clone_om_pa.clonePreCheck_om执行异常".center(60, "@"))

    # 克隆流 flowsClone(单独执行报错)
    def flowsClone_om(self):
        servicename = self.name1 + "," + self.name2
        params = {
        ...
        try:
            ...
            if codes == 200 and "completed" in info:
                loggers.info("clone_om_pa.flowsClone_om执行成功".center(60, "~"))
            else:
                loggers.info("clone_om_pa.flowsClone_om执行异常".center(60, "!"))
        except Exception as e:
            loggers.info("clone_om_pa.flowsClone_om执行异常".center(60, "@"))

    # 获取克隆进程  getCloneProgress
    def getCloneProgress_om(self):
        params = {
            "flowIds": self.follow_id,
            "batch_id": self.uuids,
            "tokenID": self.token
        ...
        if codes == 200 and flag == 0:
            loggers.info("clone_om_pa.getCloneProgress_om执行成功".center(60, "~"))
        else:
            loggers.info("clone_om_pa.getCloneProgress_om执行失败".center(60, "!"))
    ...
    def getflowid(self):
        ...
        for name in namelist:
            ...
            try:
                ...
                loggers.info("clone_om_pa.getflowid执行异常".center(60, "@"))
        return flowid_list

    # 删除克隆的任务 deleteModuleEngineFlowInfo
    def deleteModuleEngine_om(self):
        ...
        for flowid in flowidlist:
            ...
            try:
                ...
                if codes == 200 and reflowid == flowid:
                    loggers.info("clone_om_pa.deleteModuleEngine_om执行成功".center(60, "~"))
                else:
                    loggers.info("clone_om_pa.deleteModuleEngine_om执行失败".center(60, "!"))
            except Exception as e:
                loggers.info("clone_om_pa.deleteModuleEngine_om执行异常".center(60, "@"))
