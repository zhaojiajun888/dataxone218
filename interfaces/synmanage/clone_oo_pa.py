...
class Clone_oo(Publicmethods):
    """
        oracle-oracle，一对一，不开启分离
        在savelink5_oo_pa,停止运行后进行克隆,删除
        实现:克隆,删除克隆数据
    """
    def __init__(self):
        super(Clone_oo, self).__init__()
    ...
    def clonePreCheck_oo(self):
        ...
        try:
            ...
            if self.name1 == rename1 and self.name2 == rename2:
                loggers.info("Clone_oo_pa.clonePreCheck_oo执行成功".center(60, "~"))
            else:
                loggers.info("Clone_oo_pa.clonePreCheck_oo执行失败".center(60, "!"))
        except Exception as e:
            loggers.info("Clone_oo_pa.clonePreCheck_oo执行异常".center(60, "@"))

    # 克隆流 flowsClone(单独执行报错)
    def flowsClone_oo(self):
        servicename = self.name1 + "," + self.name2
        params = {
        ...
        try:
            ...
            if codes == 200 and "completed" in info:
                loggers.info("clone_oo_pa.flowsClone_oo执行成功".center(60, "~"))
            else:
                loggers.info("clone_oo_pa.flowsClone_oo执行异常".center(60, "!"))
        except Exception as e:
            loggers.info("clone_oo_pa.flowsClone_oo执行异常".center(60, "@"))

    # 获取克隆进程  getCloneProgress
    def getCloneProgress_oo(self):
        params = {
            "flowIds": self.follow_id,
            "batch_id": self.uuids,
            "tokenID": self.token
        ...
        if codes == 200 and flag == 0:
            loggers.info("clone_oo_pa.getCloneProgress_oo执行成功".center(60, "~"))
        else:
            loggers.info("clone_oo_pa.getCloneProgress_oo执行失败".center(60, "!"))
    ...
    def getflowid(self):
        ...
        for name in namelist:
            ...
            try:
                ...
                loggers.info("clone_oo_pa.getflowid执行异常".center(60, "@"))
        return flowid_list

    # 删除克隆的任务 deleteModuleEngineFlowInfo
    def deleteModuleEngine_oo(self):
        ...
        for flowid in flowidlist:
            ...
            try:
                ...
                if codes == 200 and reflowid == flowid:
                    loggers.info("clone_oo_pa.deleteModuleEngine_oo执行成功".center(60, "~"))
                else:
                    loggers.info("clone_oo_pa.deleteModuleEngine_oo执行失败".center(60, "!"))
            except Exception as e:
                loggers.info("clone_oo_pa.deleteModuleEngine_oo执行异常".center(60, "@"))
